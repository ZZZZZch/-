...
def makecard_setplayers():
    ...
    with open("题库.txt","r") as f:
        for line in f:
            ...
            words_pair.append(tmp_pair)
    random_choice = random.randint(0,len(words_pair))

    pair = words_pair[random_choice]
    print("生成的词语对为 【%s】和【%s】"%(pair[0],pair[1]))

    print("【参与人数设置】")
    player_num = int(input("请输入参与玩家数目："))
    print("玩家数为%d人……"%player_num)
    return pair, player_num
